...
object RadarChartSeverity {
    ...
    fun forSeries(value: Float, encoding: RadarIntensityEncoding): Float = when (encoding) {
        ...
        RadarIntensityEncoding.OPEN_REFLECTIVITY -> interpolate(value, regional)
        RadarIntensityEncoding.OPEN_ALPHA -> interpolate(RainAlarmPalette.fromOpenAlpha(value), regional)
    }

    /** Inverse presentation anchors used by both rain and likely-snow hues. */
    fun sharedPaletteIntensity(chartSeverity: Float): Float {
        val value = chartSeverity.coerceIn(0f, 1f)
        val palette = floatArrayOf(63f, 83f, 123f, 153f, 193f)
        for (index in 1 until chartLevels.size) if (value <= chartLevels[index]) {
            val fraction = (value - chartLevels[index - 1]) /
                (chartLevels[index] - chartLevels[index - 1])
            return (palette[index - 1] + (palette[index] - palette[index - 1]) * fraction) / 255f
        }
        return palette.last() / 255f
    }
