...
Rain Alarm 0.1.3 is a clean-room, open-source Android app for a simple question: **when will rain reach my selected place?** Now shows a place-specific rain outlook; Radar lets you inspect the source imagery and its available time window. The app does not contain the retired RainToday app, its native library, artwork, credentials or assets. Research material under `Research/RainToday` is development-only and is never packaged.

Download the [v0.1.3 signed release](https://github.com/Undert0e-505/RainAlarm/releases/tag/v0.1.3) and read its [release notes](docs/releases/v0.1.3.md) before installing. Rain Alarm is an experimental weather aid, not a safety-critical warning service.
...
- The map remembers camera zoom/pan across tabs and sessions. Switching selected places retains zoom and relative framing. Radar's centred place title shares Now's size and position; a center-on-selected button and place switcher sit close to either side. The center button preserves zoom and selection. The separate map recenter control obtains a fresh device fix and preserves zoom. Refresh reloads the active radar session and selected ancillary layer while retaining the camera. Long-press map save remains available.
...
```powershell
.\scripts\build-release.ps1 -Mode Release -Publish -Remote origin -NotesFile .\docs\releases\v0.1.3.md -WhatIf
.\scripts\build-release.ps1 -Mode Release -Publish -Remote origin -NotesFile .\docs\releases\v0.1.3.md
```
