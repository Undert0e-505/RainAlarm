...
Rain Alarm 0.1.7 is a clean-room, open-source Android app for a simple question: **when will rain reach my selected place?** Now shows a place-specific rain outlook; Radar lets you inspect the source imagery and its available time window. The app does not contain the retired RainToday app, its native library, artwork, credentials or assets. Research material under `Research/RainToday` is development-only and is never packaged.

Download the [v0.1.7 signed release](https://github.com/Undert0e-505/RainAlarm/releases/tag/v0.1.7) and read its [release notes](docs/releases/v0.1.7.md) before installing. Rain Alarm is an experimental weather aid, not a safety-critical warning service.
...
```powershell
.\scripts\build-release.ps1 -Mode Release -Publish -Remote origin -NotesFile .\docs\releases\v0.1.7.md -WhatIf
.\scripts\build-release.ps1 -Mode Release -Publish -Remote origin -NotesFile .\docs\releases\v0.1.7.md
```
