...
Rain Alarm 0.1.8 is a clean-room, open-source Android app for a simple question: **when will rain reach my selected place?** Now shows a place-specific rain outlook; Radar lets you inspect the source imagery and its available time window. The app does not contain the retired RainToday app, its native library, artwork, credentials or assets. Research material under `Research/RainToday` is development-only and is never packaged.

Download the [v0.1.8 signed release](https://github.com/Undert0e-505/RainAlarm/releases/tag/v0.1.8) and read its [release notes](docs/releases/v0.1.8.md) before installing. Rain Alarm is an experimental weather aid, not a safety-critical warning service.
...
The unset defaults are Rain Notification **on when permission allows**, all five Now weather indicators visible, radar playback **2×**, MeteoGroup regional radar, and dark app/dark map. Radar playback still starts **paused**. Settings can change the radar source, speed, app/map appearance, independent Compass-card and Graph-card appearance (each follows the app by default), visible weather indicators, alert toggle and default startup place. When Open radar is explicitly selected, the additional **Show likely snow** switch is available and defaults off. The Radar Layers dropdown remembers its own Off/Wind/Lightning/Fog choice; Off loads no ancillary layer.
...
- MeteoGroup/DTN regional radar is the default in supported European areas. It supplies map observations, velocity textures and provider forecast frames. For regional Now/alerts, a separate location-selected **area profile** supplies average/minimum/maximum intensity when fresh; it is not exact pin-pixel truth. The raster point series is the fallback. RainViewer is a worldwide open-radar choice and automatic session fallback. RainViewer RGB is decoded from its published Universal Blue reflectivity colours into the same Rain Alarm intensity palette and animation treatment; PNG alpha is transparency, never precipitation strength or proof of radar coverage. A separate RainViewer coverage mask is requested once per selected-point analysis. A fresh tile with no above-threshold echoes is reported as a clear next hour without inventing a direction only when that mask confirms the selected point and substantially the whole retained tile are covered. Missing/stale/invalid coverage stays unavailable. When Open radar is explicitly selected, optional **Show likely snow** requests RainViewer's model-assisted snow colours and uses an icy lavender-to-indigo hue across Radar, Now, the graph and alert wording. It stays off for automatic fallback. RainViewer's clean-room future path prefers a confidence-gated local motion field and can use a separately confidence-gated broad translation when the local field is unavailable; both are estimates, not provider forecasts. An estimate stops at the downloaded coverage edge rather than treating unseen space as dry. MeteoGroup and RainViewer are independent composites and can materially disagree about precipitation footprint, intensity and timing; the app calibrates their presentation but never expands, shrinks or moves one provider's echoes to imitate the other. Source/fallback and unavailable states are shown rather than quietly asserted as dry.
...
```powershell
.\scripts\build-release.ps1 -Mode Release -Publish -Remote origin -NotesFile .\docs\releases\v0.1.8.md -WhatIf
.\scripts\build-release.ps1 -Mode Release -Publish -Remote origin -NotesFile .\docs\releases\v0.1.8.md
```
